#include "stdafx.h"
#include "EnclaveTriangleOld.h"
#include "PrimaryLineT1Array.h"

EnclaveTriangleOld::EnclaveTriangleOld()
{

}

EnclaveTriangleOld::EnclaveTriangleOld(FTriangleOutput in_fTriangleOutput)
{
	/*
...
void EnclaveTriangleOld::executeRun(BlockBorderLineList* in_blockBorderLineList,
...
void EnclaveTriangleOld::executeRunDebug(BlockBorderLineList* in_blockBorderLineList,
	BorderDataMap* in_borderDataMap,
	EnclaveKeyDef::EnclaveKey in_key)
{
	/*
...
void EnclaveTriangleOld::purgeBadFans()
{
	/*
...
void EnclaveTriangleOld::runBoundaryOrientationPass()
{
	/*
	auto fansBegin = enclaveTriangleTertiary.triangleMap.begin();
	auto fansEnd = enclaveTriangleTertiary.triangleMap.end();
...
void EnclaveTriangleOld::resetRunMetaData()
{
	/*
...
ECBPolyPoint EnclaveTriangleOld::convertDoublePointToECBPolyPointForEnclaveTriangle(DoublePoint in_doublePointToConvert)
{
...
PrimaryLineT1Array EnclaveTriangleOld::generatePrimaryLineT1Array(PolyRunDirection in_polyRunDirection)
{
...
void EnclaveTriangleOld::printExteriorBlocks()
{
...
void EnclaveTriangleOld::printPoints()
{
	for (int x = 0; x < 3; x++)
	{
...
void EnclaveTriangleOld::printBlockKeys()
{
...
void EnclaveTriangleOld::executeForwardRunTest(PrimaryLineT1Array* in_linkArrayRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap)
{
	for (int x = 0; x < 3; x++)
	{
...
void EnclaveTriangleOld::runPolyLinesThroughEnclave(PrimaryLineT1Array* in_linkArrayRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap)
{
	...
	for (int x = 0; x < 3; x++)
	{
		...
		while (currentLineRunner.isRunComplete == 0)
		{
			...
			{
				std::cout << "(EnclaveTriangle): attempt to add segment failed during call to EnclaveTriangleOld::runPolyLinesThroughEnclave." << std::endl;
...
void EnclaveTriangleOld::runPolyLinesThroughEnclaveReverse(PrimaryLineT1Array* in_linkArrayRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap)	// run the exterior lines
{
	/*
	for (int x = 0; x < 3; x++)
	{
		//std::cout << "####### spawning EnclaveLineRunner from EnclaveTriangleOld::runPolyLinesThroughEnclaveReverse" << std::endl;
		EnclaveLineRunner currentLineRunner(in_linkArrayRef->linkArray[x], in_blockBorderLineList, x);
		while (currentLineRunner.isRunComplete == 0)
		{
			currentLineRunner.checkForCompletion();
...
void EnclaveTriangleOld::runInteriorRunnerReverse(PrimaryLineT1Array* in_linkArrayRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap, PolyRunDirection in_polyRunDirection)
{
	/*
	TerminatingSetContainer* terminatingSetRef = &reverseTerminatingContainer;	// will be one of two values below
	for (int x = 0; x < 3; x++)
	{
		//std::cout << "####### spawning EnclaveLineRunner from EnclaveTriangleOld::runInteriorRunnerReverse" << std::endl;
		EnclaveLineRunner currentLineRunner(in_linkArrayRef->linkArray[x], in_blockBorderLineList, x);
		...
		while (currentLineRunner.isRunComplete == 0)
		{
			...
			if (bbFanRef->reverseSegmentTracker.currentLineSegmentIndex == 2)
			{
						...
						)
				{
					...
					{
						//std::cout << "Triangle flagged as invalid, in EnclaveTriangleOld::runInteriorRunnerReverse; flagging as INVALID. " << std::endl;
						isTriangleValid = false;
						break;
					}
...
void EnclaveTriangleOld::runPolyLinesForReverseTerminatingContainer(PrimaryLineT1Array* in_linkArrayRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap)
{
	/*
	int limitCount = 0;
	for (int x = 0; x < 3; x++)
	{
		//std::cout << "####### spawning EnclaveLineRunner from EnclaveTriangleOld::runPolyLinesForReverseTerminatingContainer" << std::endl;
		//std::cout << "### Line: " << x << std::endl;
		EnclaveLineRunner currentLineRunner(in_linkArrayRef->linkArray[x], in_blockBorderLineList, x);
		while (currentLineRunner.isRunComplete == 0)
		{
...
void EnclaveTriangleOld::runInteriorRunners(PrimaryLineT1Array* in_linkArrayRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap, PolyRunDirection in_polyRunDirection)
{
	/*
	...
	for (int x = 0; x < 3; x++)
	{
				...
				)
		{
			...
			if (bbFanRef->blockSegmentTracker.currentLineSegmentIndex == 1)	// if it only contains 1 segment, do the run.
			{
				...
				if (IndependentUtils::checkIfInterceptIsValid(interceptToUse) == false)
				{
					std::cout << "Triangle flagged as invalid, in EnclaveTriangleOld::runInteriorRunners (1 segment); flagging as INVALID. " << std::endl;
					isTriangleValid = false;
					break;
				}
			...
			if (bbFanRef->blockSegmentTracker.currentLineSegmentIndex == 2)
			{
				...
				if (in_polyRunDirection == PolyRunDirection::NORMAL)
				{
					...
					if (result.isValid == 1)
					{
						...
						if (IndependentUtils::checkIfInterceptIsValid(interceptToUse) == false)
						{
							std::cout << "Triangle flagged as invalid, in EnclaveTriangleOld::runInteriorRunners (2 segments); flagging as INVALID. " << std::endl;
							//isTriangleValid = false;
							//break;
						}
...
void EnclaveTriangleOld::runInteriorRunnersDebug(PrimaryLineT1Array* in_linkArrayRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap, PolyRunDirection in_polyRunDirection)
{
	/*
	//std::cout << "Poly perfect clamp value is: " << IsEnclaveTrianglePolyPerfectlyClamped << std::endl;

	//std::cout << "#################### spawning EnclaveLineRunners from EnclaveTriangleOld::runInteriorRunners" << std::endl;
	...
	for (int x = 0; x < 3; x++)
	{
		std::cout << "(EnclaveTriangleOld::runInteriorRunnersDebug) ################## Attempting interior runner from line: " << x << std::endl;
...
bool EnclaveTriangleOld::isSegmentAndSlopeParallel(SegmentMeta in_segment, ECBPolyPoint in_slope)
{
	bool result = false;
...
bool EnclaveTriangleOld::doBlocksExistAtY(int in_y)
{
...
void EnclaveTriangleOld::reform(ECBPolyPoint in_polyPoint0, ECBPolyPoint in_polyPoint1, ECBPolyPoint in_polyPoint2)
{
	TriangleLine line1, line2, line3;
...
void EnclaveTriangleOld::generateExteriorLineSegmentsET(PrimaryLineT1Array* in_linkArrayRef,
...
void EnclaveTriangleOld::fillCircuitMetaData(BlockCircuit* in_circuitRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap, EnclaveKeyDef::EnclaveKey in_key)
{
	/*
...
void EnclaveTriangleOld::fillCircuitMetaDataAndCheckValidity(BlockCircuit* in_circuitRef, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap, EnclaveKeyDef::EnclaveKey in_key)
{
	/*
...
void EnclaveTriangleOld::populateOrganicWrappedBBFanWithCircuitData(BlockCircuit* in_circuitRef, int in_secondaryID)
{
	/*
...
int EnclaveTriangleOld::findIndexOfSegmentWithNonFreeEndpoint(BlockCircuit* in_circuitRef, EnclaveKeyDef::EnclaveKey in_key)
{
...
bool EnclaveTriangleOld::isEndpointValidForFreeSelection(int in_lineID, BlockCircuit* in_circuitRef)
{
	bool result = false;
...
ECBPolyPoint EnclaveTriangleOld::getTraceResultForEndpointSegment(BlockCircuit* in_circuitRef, int in_segmentIndex, BlockBorderLineList* in_blockBorderLineList, BorderDataMap* in_borderDataMap)
{
...
void EnclaveTriangleOld::performCentroidBlockCheck(ECBPolyPoint in_pointA, ECBPolyPoint in_pointB, ECBPolyPoint in_pointC)
{
