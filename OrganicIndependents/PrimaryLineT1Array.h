...
public:
	...
	void adjustForEnclavePerfectClamping(PerfectClampEnum in_perfectClampValue);	// needed by EnclaveTriangle, when calling
	...
	void produceT2Lines(EnclaveKeyDef::EnclaveKey in_key, 
	...
	void runT2LinesMulti(EnclaveKeyDef::EnclaveKey in_key, 
						ECBPolyLineSlopesArray* in_polyLineSlopesPtr, 
						ECBBorderLineList* in_blueprintBorderLines, 
						PrimaryLineT2Array* in_primaryLineT2ArrayRef, 
						PerfectClampEnum in_perfectClampFlag);			// generates T2 lines by using a multi-line trace (when there is more than one line)
