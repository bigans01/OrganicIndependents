...
std::string HashUtils::convertFTrianglePointToHash(FTrianglePoint in_pointToConvert)
{
	// Step 1: multiply each float by 100 (to get rid of hundredths deciaml), then convert to int.
	int xConverted = static_cast<int>(in_pointToConvert.point.x * 100);
	int yConverted = static_cast<int>(in_pointToConvert.point.y * 100);
	int zConverted = static_cast<int>(in_pointToConvert.point.z * 100);
